...
``Image.priority`` (the anomaly_first ordering key). Web-safe (stdlib only) and
idempotent; both ``manage prioritize`` and the ingest API's ``POST /api/prioritize``
delegate to :func:`run`.
...
def run() -> int:
    """Rescore every planned step's images; return how many changed."""
    active = plan.active()
    updated = 0
    for step_plan in active.steps:
        ...
        values = dict(
            models.Metric.objects.filter(file__step=step, name=order_by).values_list(
                "file__file1", "value"
            )
        )
