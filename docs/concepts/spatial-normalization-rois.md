...
| sulcal/fissure/tentorium bands | hand-drawn Benhajali landmarks (no atlas defines these), shipped on the canonical `MNI152NLin2009cAsym` grid; warped per space | canonical + registered spaces |
...
- **Per-space registration**: for a non-canonical space, `rois.build_rois`
  registers the canonical template's masked T1w to the target's (dipy
  affine(MI) + SyN(CC)) and warps the landmark labels with nearest-neighbour
  interpolation. The build **fails** unless warping improves the masked-T1w
  correlation over plain world-coordinate resampling — a mis-applied transform
  cannot slip through silently.
- **Why not TemplateFlow's published `*_xfm.h5` composites?** They were
  evaluated and rejected: applied through nitransforms, their
  displacement-field component *lowered* masked-T1w correlation below identity
  (0.66 vs 0.78 for MNI152NLin6Asym; the validated dipy registration reaches
  0.93), indicating a field-convention mismatch in the reader. Computing the
  registration keeps one engine, validated by construction.
- **Provenance**: the artifact sidecar records the algorithm version, band
  width, per-structure source, every input file's sha256, the registration
  engine/parameters, and the r-improvement numbers. Post-warp label cleanup
  (one closing + largest component, to repair nearest-neighbour raggedness) is
  recorded per structure and refuses to change any structure by more than 10%.
...
Building needs the network (TemplateFlow) and, for non-canonical spaces, a few
minutes of registration. Offline compute nodes therefore **pre-warm on a login
node**:
...
3. Run `manage build_rois <space>` and inspect the built dseg over the
   template T1w before trusting it.
